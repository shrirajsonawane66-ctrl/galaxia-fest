...
The interactive core in `src/components/SolarSystem.tsx` is driven by one `requestAnimationFrame` loop. Its feel is tuned with `IDLE=6`, `MAX_SPEED=900`, `RAMP_PER_SEC=180`, `EASE=2.4`, `SYS_IDLE=10`, and `SYS_FOLLOW=0.85`. Held target speed grows as `IDLE + heldSeconds² * RAMP_PER_SEC`; normalized `speedFactor` drives ripple spacing (`0.62 - speedFactor * 0.5` seconds), ripple duration/scale, system speed, and audio playback rate. Reduced-motion users get no ripples and a `120` deg/sec cap. The looping track is served as the cacheable static asset `/audio/galaxia-prime.mp3`.
